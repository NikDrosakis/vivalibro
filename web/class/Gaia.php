...
class Gaia extends DB{
	...
	public $my;
	public $globalfile;
	public function __construct($database='',$dbt='maria'){
				$this->my=$_COOKIE;						
				...
				//	if(file_exists('gaia.json')){
//xecho($_SERVER['SERVER_NAME']);						
					$this->db=new DB($database,$dbt,$_SERVER['SERVER_NAME']);
...
//             }
     ...
     * LOGIN EVENT DATA
     * */
    public function login($pass,$mail){
        $fetch=$this->db->f("SELECT * FROM user WHERE mail=? AND pass=?",array($mail,$pass));
...
//                $col= $typ2table.' '.$typ2row;
	...
	if($_GET['page']=="book"){		
		$qq=$q!=""
			? "WHERE bookuser.libid=? AND (book.title LIKE '%$q%' 
			OR writer.name LIKE '%$q%' 
			OR cat.name LIKE '%$q%' 
			OR editor.name LIKE '%$q%')"
			:"";
		$query="SELECT book.*, CONCAT('/book?id=',book.id) as booklink,writer.name as writername,cat.name as catname,editor.name as editorname 
		FROM bookuser 
		left join book on bookuser.bookid=book.id 
		left join writer on book.writer=writer.id 
		left join cat on book.cat=cat.id 
		left join editor on book.editor=editor.id		
		$qq ORDER BY $orderby ";
		$sel= $bot->fa("$query $limit",$my["libid"]);

	}elseif($_GET['page']=="writer"){			
		$query="SELECT * FROM writer ORDER BY name desc";
		$list= $bot->fa("$query $limit");
		$sel=[];
		for($i=0;$i<count($list);$i++){		
		$sel[$i]=$list[$i];	
		$id=(int)$list[$i]['id'];
		$sel[$i]['books']=$bot->fl(["id","title"],"book","WHERE writer=$id");
		$sel[$i]['categories']=$bot->fl(["cat.id","cat.name"],"cat","LEFT JOIN book ON book.cat=cat.id WHERE book.writer=$id");
		}
	}elseif($_GET['page']=="editor"){			
		$query="SELECT * FROM editor ORDER BY name desc";
		$list= $bot->fa("$query $limit");
		$sel=[];
		for($i=0;$i<count($list);$i++){		
		$sel[$i]=$list[$i];	
		$id=(int)$list[$i]['id'];
		$sel[$i]['books']=$bot->fl(["id","title"],"book","WHERE editor=$id");
		$sel[$i]['categories']=$bot->fl(["cat.id","cat.name"],"cat","LEFT JOIN book ON book.cat=cat.id WHERE book.writer=$id");
		}
}
	...
	for($i=0;$i<count($sel);$i++){		 		
			$buffer['titles'][$i]= $_GET['page']=="book" ? $sel[$i]['title'] : $sel[$i]['name'];    
	}		
	}
	if($_COOKIE['boxstyle']=='table'){
		$buffer['html']=include_buffer(SITE_ROOT."pages/".$_GET['page']."_table.php",$sel);
	}elseif(empty($_COOKIE['boxstyle']) || $_COOKIE['boxstyle']=='archieve'){
		$buffer['html']=include_buffer(SITE_ROOT."pages/".$_GET['page']."_archive.php",$sel);
	}				
	return $buffer;
	}
